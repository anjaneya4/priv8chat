...
function PeerServer(options, callback) {
  ...
  app.use('/public', express.static(pathlib.join(__dirname, '../public')));
  console.log(pathlib.join(__dirname, '../public'));
  var peerjs = ExpressPeerServer(server, options);
  app.use(path, peerjs);
  app.get('/send', function(req, res) {
      res.sendFile(pathlib.join(__dirname, '../public/send.html'));
  });
  app
  app.set('views', pathlib.join(__dirname, '../public'))
     .set('view engine', 'ejs')
     .get('/chat/:id', (req, res) => res.render('conversation', {peer_id: req.params.id,
                                                                  peer_server: 'priv8chat.herokuapp.com',
                                                                  peer_port: port}))
     .get('/chat', (req, res) => res.render('receive', {peer_id: req.params.id,
                                                                  peer_server: 'priv8chat.herokuapp.com',
                                                                  peer_port: port}))
  if (callback) {
